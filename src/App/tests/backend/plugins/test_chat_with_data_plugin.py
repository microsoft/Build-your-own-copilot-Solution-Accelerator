...
class TestChatWithDataPlugin:
    ...
    def test_greeting_returns_response(self, mock_get_openai_client):
        ...
        mock_completion.choices[0].message.content = "Hello! I'm your Wealth Assistant. How can I help you today?"
    ...
    def test_get_openai_client_success(self, mock_default_credential, mock_token_provider, mock_azure_openai, mock_config):
    ...
    @patch('backend.plugins.chat_with_data_plugin.DefaultAzureCredential')
    def test_get_project_openai_client_success(self, mock_default_credential, mock_ai_project_client, mock_config):
    ...
    @patch.object(ChatWithDataPlugin, 'get_openai_client')
    def test_get_sql_response_success(self, mock_get_openai_client, mock_get_connection):
        ...
        mock_completion = MagicMock()
        mock_completion.choices = [MagicMock()]
        mock_completion.choices[0].message.content = "SELECT * FROM Clients WHERE ClientId = 'client123';"
        mock_client.chat.completions.create.return_value = mock_completion
    ...
    @patch('backend.plugins.chat_with_data_plugin.get_connection')
    @patch.object(ChatWithDataPlugin, 'get_openai_client')
    def test_get_sql_response_database_error(self, mock_get_openai_client, mock_get_connection):
    ...
    def test_get_answers_from_calltranscripts_success(self, mock_get_openai_client):
        ...
        mock_completion = MagicMock()
        mock_completion.choices = [MagicMock()]
        mock_completion.choices[0].message.content = "Based on call transcripts, the customer discussed investment options and risk tolerance."
        mock_client.chat.completions.create.return_value = mock_completion
        
        result = self.plugin.get_answers_from_calltranscripts("What did the customer discuss?", "client123")
    ...
    def test_get_answers_from_calltranscripts_no_results(self, mock_get_openai_client):
        ...
        result = self.plugin.get_answers_from_calltranscripts("Nonexistent query", "client123")
        
        assert "No data found for that client." in result

    @patch.object(ChatWithDataPlugin, 'get_openai_client')
    def test_get_answers_from_calltranscripts_openai_error(self, mock_get_openai_client):
        """Test call transcripts with OpenAI processing error."""
        mock_client = MagicMock()
        mock_get_openai_client.return_value = mock_client
        
        # Simulate OpenAI error
        mock_client.chat.completions.create.side_effect = Exception("OpenAI processing failed")
