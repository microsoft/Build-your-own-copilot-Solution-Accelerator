...
class GrantsPage(BasePage):
    GRANTS_ICON = "//button[@aria-label='button']//following-sibling::span[text()='Grants']"
    GRANTS_PAGE_TITLE = "//div/h2[text()='Explore grant documents']"
    ...
    CHAT_HISTORY_QUESTION_TEXT = "//div[text() = 'are there any grants that focus on clinical research concerning influenza vaccination?']"
    CLEAR_CHAT_HISTORY = "//button[@aria-label='clear chat button']"

    def __init__(self, page):
        self.page = page

    def click_grants_icon(self):
        # click on Articles icon on welcome page
        self.page.locator(self.GRANTS_ICON).click()
