...
describe('ChatHistoryListItemCell', () => {
  ...
  test('truncates long title', () => {
    ...
    renderWithContext(
      <ChatHistoryListItemCell item={longTitleConversation} onSelect={mockOnSelect} />,
      mockAppState
    );
  ...
  test('when null item is not passed', () => {
    renderWithContext(
      <ChatHistoryListItemCell onSelect={mockOnSelect} />,
      mockAppState
    );
  ...
  test('handles rename save when the updated text is equal to initial text', async () => {
    ...
    await waitFor(() => expect(screen.queryByText('Error: Enter a new title to proceed.')).not.toBeInTheDocument(), { timeout: 6000 });
  ...
  test('handles rename save API failed', async () => {
    ...
    // Wait for the error to be hidden after 5 seconds
    await waitFor(() => expect(screen.queryByText('Error: could not rename item')).not.toBeInTheDocument(), { timeout: 6000 });
    const input = screen.getByLabelText('rename-input');
    expect(input).toHaveFocus();
  }, 10000);
