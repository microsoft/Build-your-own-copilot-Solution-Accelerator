...
jest.mock('../../components/Cards/Cards', () => { return jest.fn((props: any) => <div data-testid="user-card-mock" onClick={() => props.onCardClick(mockUsers)}>Mocked Card Component</div>); });

jest.mock('../chat/Chat', () => {
    const Chat = () => (
      <div data-testid='Chat-component'>Mocked Chat Component</div>
    );
    return Chat;
})
...
test('check the welcomeCard', async () => {
    ...
    expect(screen.getByText('You can ask questions about their portfolio details and previous conversations or view their profile.')).toBeVisible()
})
