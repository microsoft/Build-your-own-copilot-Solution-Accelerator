...
import { getUserInfo, conversationApi,historyGenerate, historyClear, ChatMessage, Citation, historyUpdate, CosmosDBStatus } from '../../api';
    ...
    ChatMessageContainer: jest.fn((props: ChatMessageContainerProps) => {
        return (
            <div data-testid="chat-message-container">
                ...
                <button aria-label={"citation-btn"} onClick={() => props.onShowCitation(citationObj)}> Show Citation</button>
    ...
    CitationPanel: jest.fn((props: any) => {
        return (
            <>
                ...
                <button aria-label="bobURL" onClick={() => props.onViewSource(props.activeCitation)} >BOB URL</button>
            </>
        )
    }),
}));
...
jest.mock('../../components/QuestionInput', () => ({
    QuestionInput: jest.fn((props:any) => <div>
        <span>QuestionInputMock</span>
        <button aria-label ="question-input" onClick={()=>props.onSend(
            'List of Documents', props.conversationId
        )}>Click</button>

        <button aria-label ="question-dummy" onClick={()=>props.onSend(
...
jest.mock('../../components/PromptsSection/PromptsSection', () => ({
    PromptsSection: jest.fn((props: any) => <div role="button" aria-label={"prompt-button"} onClick={() => props.onClickPrompt(
...
describe("Chat Component", () => {
    ...
    test('Should show Auth not configured when userList length zero', async () => {
        ...
        renderWithContext(<Chat />, mockState)
        await waitFor(() => {
    ...
    test("should handle historyGenerate API failure correctly", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText(/There was an error generating a response. Chat history can't be saved at this time. Please try again/i)).toBeInTheDocument();
        })
    ...
    test('Should handle historyGenerate API returns empty object or null', async () => {
        ...
        await waitFor(async () => {
            expect(screen.getByText(/There was an error generating a response. Chat history can't be saved at this time./i)).toBeInTheDocument();
        })
    ...
    test("Should open error dialog when handle historyClear failure ", async () => {
        ...
        await waitFor(async () => {
            ...
            expect(await screen.findByText(/Please try again. If the problem persists, please contact the site administrator./i)).toBeInTheDocument();
        })
    })
    ...
    test("Should handle historyUpdate API return ok as false", async () => {
        ...
        await waitFor(async () => {
            expect(await screen.findByText(/An error occurred. Answers can't be saved at this time. If the problem persists, please contact the site administrator./i)).toBeInTheDocument();
        })
    })
