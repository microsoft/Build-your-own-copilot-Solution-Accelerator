...
const documentSectionData = [
  {
    ...
    metaPrompt: "Meta for Introduction",
  },
  {
    title: "Methods",
    content: "Methods content here.",
    metaPrompt: "Meta for Methods",
  },
];

const renderComponent = (props = {}) => {
  return renderWithContext(
    <QuestionInput onSend={mockOnSend} disabled={false} {...props} />
  );
};
...
describe("QuestionInput Component", () => {
  ...
  it("should set the initial question and dispatch when showInitialChatMessage is true", () => {
    ...
    const { getByRole } = renderWithContext(
      <QuestionInput onSend={mockOnSend} disabled={false} />,
      mockState
    );
    ...
    expect(mockDispatch).toHaveBeenCalledWith({
      type: "SET_SHOW_INITIAL_CHAT_MESSAGE_FLAG",
      payload: false,
    });
  });
});
