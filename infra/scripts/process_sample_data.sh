...
get_values_from_azd_env() {
	...
	# Allow only valid characters in the names, so that error and warning messages are avoided
	resourceGroupName=$(azd env get-value RESOURCE_GROUP_NAME 2>&1 | grep -E '^[a-zA-Z0-9._/-]+$')
	cosmosDbAccountName=$(azd env get-value COSMOSDB_ACCOUNT_NAME 2>&1 | grep -E '^[a-zA-Z0-9._/-]+$')
	...
	sqlServerName=$(azd env get-value SQLDB_SERVER_NAME 2>&1 | grep -E '^[a-zA-Z0-9._/-]+$')
	SqlDatabaseName=$(azd env get-value SQLDB_DATABASE 2>&1 | grep -E '^[a-zA-Z0-9._/-]+$')
	sqlManagedIdentityClientId=$(azd env get-value MANAGEDIDENTITY_SQL_CLIENTID 2>&1 | grep -E '^[a-zA-Z0-9._/-]+$')
	sqlManagedIdentityDisplayName=$(azd env get-value MANAGEDIDENTITY_SQL_NAME 2>&1 | grep -E '^[a-zA-Z0-9._/-]+$')
	aiSearchName=$(azd env get-value AI_SEARCH_SERVICE_NAME 2>&1 | grep -E '^[a-zA-Z0-9._/-]+$')
...
get_values_from_user() {
	...
	if [ -z "$cosmosDbAccountName" ] || [ -z "$storageAccount" ] || [ -z "$fileSystem" ] || [ -z "$keyvaultName" ] || [ -z "$sqlServerName" ] || [ -z "$SqlDatabaseName" ] || [ -z "$sqlManagedIdentityClientId" ] || [ -z "$sqlManagedIdentityDisplayName" ] || [ -z "$aiSearchName" ] || [ -z "$aif_resource_id" ]; then
		...
		echo "All values provided successfully."
		return 0
	fi
}
...
echo "Cosmos DB Account Name: $cosmosDbAccountName"
